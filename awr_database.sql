-- ============================================================
-- AWR KEY SYSTEM v6.1 — DATABASE SETUP LENGKAP
-- PASTE DI SUPABASE SQL EDITOR → RUN
-- Aman dijalankan berkali-kali (idempotent, tidak hapus data)
-- v6.1 changelog: + blockblast_settings, + blockblast_scores
-- ============================================================
...
CREATE TABLE IF NOT EXISTS users (
  ...
  role                VARCHAR(20)  DEFAULT 'user'
                        CHECK (role IN ('user', 'reseller', 'developer')),
  is_banned           BOOLEAN      DEFAULT false,
  ban_reason          TEXT,
  avatar_url          TEXT,
  avatar_file_url     TEXT,
  background_url      TEXT,
  background_type     VARCHAR(10)  DEFAULT 'image'
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='users' AND column_name='avatar_file_url') THEN
    ALTER TABLE users ADD COLUMN avatar_file_url TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='users' AND column_name='leaderboard_public') THEN
    ALTER TABLE users ADD COLUMN leaderboard_public BOOLEAN DEFAULT true;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='users' AND column_name='support_contact') THEN
    ALTER TABLE users ADD COLUMN support_contact TEXT;
  END IF;
END $$;

-- ============================================================
-- SECTION 3: TABLE KEYS
-- ============================================================
CREATE TABLE IF NOT EXISTS keys (
  ...
  duration_type  VARCHAR(20)  DEFAULT '24h'
                   CHECK (duration_type IN ('24h','3d','5d','7d','30d','60d','lifetime')),
  created_at     TIMESTAMPTZ  DEFAULT NOW(),
  updated_at     TIMESTAMPTZ  DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS idx_key_hwids_hwid   ON key_hwids(hwid);

-- ============================================================
-- SECTION 5: TABLE EXECUTION_LOGS
-- ============================================================
...
-- ============================================================
-- SECTION 7: TABLE FEEDBACKS
-- ============================================================
CREATE TABLE IF NOT EXISTS feedbacks (
  id                  UUID         DEFAULT gen_random_uuid() PRIMARY KEY,
  type                VARCHAR(50)  NOT NULL
                        CHECK (type IN ('Saran', 'Report Bug', 'Feedback')),
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  type       VARCHAR(20)  DEFAULT 'info'
               CHECK (type IN ('info','success','warning','key','announcement','key_received')),
...
-- Upgrade: tambah type 'key_received' kalau belum ada
DO $$ BEGIN
...
-- ============================================================
-- SECTION 11: TABLE PASSWORD_RESETS
-- ============================================================
...
-- ============================================================
-- SECTION 12: TABLE GETKEY_SETTINGS
-- ============================================================
...
-- ============================================================
-- SECTION 13: TABLE MUSIC_SETTINGS
-- ============================================================
CREATE TABLE IF NOT EXISTS music_settings (
  id         UUID         DEFAULT gen_random_uuid() PRIMARY KEY,
  url        TEXT         NOT NULL DEFAULT '',
  type       VARCHAR(10)  DEFAULT 'url'
               CHECK (type IN ('url', 'upload')),
  file_data  TEXT,
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='music_settings' AND column_name='file_data') THEN
    ALTER TABLE music_settings ADD COLUMN file_data TEXT;
  END IF;
END $$;
...
-- ============================================================
-- SECTION 14: TABLE SUPPORT_SETTINGS
-- ============================================================
...
INSERT INTO support_settings (whatsapp_url, telegram_url, discord_url, custom_label, is_active)
SELECT '', '', '', 'Hubungi Support', true
WHERE NOT EXISTS (SELECT 1 FROM support_settings);

-- ============================================================
-- SECTION 15: TABLE BLOCKBLAST_SETTINGS (v6.1 NEW)
-- ============================================================
...
-- ============================================================
-- SECTION 16: TABLE BLOCKBLAST_SCORES (v6.1 NEW)
-- Satu user satu baris — upsert kalau skor baru lebih tinggi
-- ============================================================
...
-- RLS untuk blockblast
ALTER TABLE blockblast_settings ENABLE ROW LEVEL SECURITY;
ALTER TABLE blockblast_scores   ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "svc_bb_settings" ON blockblast_settings;
CREATE POLICY "svc_bb_settings" ON blockblast_settings FOR ALL TO service_role USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "pub_bb_settings" ON blockblast_settings;
CREATE POLICY "pub_bb_settings" ON blockblast_settings FOR SELECT TO anon USING (true);

DROP POLICY IF EXISTS "svc_bb_scores" ON blockblast_scores;
CREATE POLICY "svc_bb_scores" ON blockblast_scores FOR ALL TO service_role USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "pub_bb_scores" ON blockblast_scores;
CREATE POLICY "pub_bb_scores" ON blockblast_scores FOR SELECT TO anon USING (true);
...
BEGIN
  SELECT json_build_object(
    ...
    'blockblast_players',(SELECT COUNT(*) FROM blockblast_scores),
    'blockblast_active', (SELECT event_active FROM blockblast_settings WHERE id = 1)
  ) INTO result;
  RETURN result;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
DROP POLICY IF EXISTS "ins_feedbacks" ON feedbacks;
CREATE POLICY "ins_feedbacks" ON feedbacks  FOR INSERT TO anon WITH CHECK (true);

DROP POLICY IF EXISTS "svc_notifs"   ON notifications;
CREATE POLICY "svc_notifs"   ON notifications  FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "svc_announce" ON announcements;
CREATE POLICY "svc_announce" ON announcements  FOR ALL TO service_role USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "pub_announce" ON announcements;
CREATE POLICY "pub_announce" ON announcements  FOR SELECT TO anon USING (is_active = true);

DROP POLICY IF EXISTS "svc_sessions" ON sessions;
...
DROP POLICY IF EXISTS "svc_music"    ON music_settings;
CREATE POLICY "svc_music"    ON music_settings  FOR ALL TO service_role USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "pub_music"    ON music_settings;
CREATE POLICY "pub_music"    ON music_settings  FOR SELECT TO anon USING (true);

DROP POLICY IF EXISTS "svc_support"  ON support_settings;
CREATE POLICY "svc_support"  ON support_settings  FOR ALL TO service_role USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "pub_support"  ON support_settings;
CREATE POLICY "pub_support"  ON support_settings  FOR SELECT TO anon USING (true);

-- ============================================================
-- SECTION 20: SEED DATA — AKUN DEVELOPER
-- ============================================================
INSERT INTO users (username, email, password_hash, role)
VALUES (
  'icansayangara',
  'developer@awr.local',
  '$2b$10$EixZaYVK1fsbw1ZfbX3OXePaWxn96p36Wa9E.LAMiKHiUKmqKTLyK',
  'developer'
)
...
-- ============================================================
-- SECTION 21: SEED DATA — GETKEY STEPS
-- ============================================================
INSERT INTO getkey_settings (name, url, duration_seconds, order_index, is_active)
VALUES
  ('Buka Link Iklan',       'https://moneyblink.com/st/?api=b238837b14e9101a5fdb857decf8238aa217c3db&url=https://msanzxmzz.vercel.app/', 30, 1, true),
  ('Join Saluran Telegram', 'https://t.me/sanzxmzz',            25, 2, true),
  ('Follow TikTok',         'https://tiktok.com/@sanzxmzz',     25, 3, true)
ON CONFLICT DO NOTHING;

-- ============================================================
-- SECTION 22: SEED DATA — ANNOUNCEMENT AWAL
-- ============================================================
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM announcements WHERE title = 'Selamat Datang di AWR Key System!') THEN
    INSERT INTO announcements (title, content, is_active)
    VALUES (
      'Selamat Datang di AWR Key System!',
      'AWR Key System v6.1 sudah online! Daftar sekarang dan dapatkan key gratis 24 jam. Event Block Blast aktif — raih skor tertinggi & menangkan key VIP! 🏆',
      true
    );
  END IF;
END $$;
...
SELECT
  ...
  u.total_executions, u.created_at,
  k.key_value, k.expires_at, k.duration_type, k.is_active AS key_active
FROM users u
LEFT JOIN keys k
  ON k.assigned_to = u.id
...
CREATE OR REPLACE VIEW v_keys_expiring_soon AS
SELECT
  k.id, k.key_value, k.expires_at, k.duration_type,
  u.username, u.email
FROM keys k
JOIN users u ON u.id = k.assigned_to
WHERE k.is_active = true
  AND k.expires_at IS NOT NULL
  AND k.expires_at BETWEEN NOW() AND NOW() + INTERVAL '24 hours'
ORDER BY k.expires_at ASC;

CREATE OR REPLACE VIEW v_leaderboard AS
SELECT
  ROW_NUMBER() OVER (ORDER BY total_executions DESC) AS rank,
  username,
  roblox_username,
  total_executions,
  avatar_url,
...
SELECT
  COUNT(*)                                           AS total,
  COUNT(*) FILTER (WHERE type = 'Report Bug')        AS bugs,
  COUNT(*) FILTER (WHERE type = 'Saran')             AS saran,
  COUNT(*) FILTER (WHERE type = 'Feedback')          AS general,
  ROUND(AVG(rating)::numeric, 1)                     AS avg_rating,
  COUNT(*) FILTER (WHERE created_at >= CURRENT_DATE) AS today
FROM feedbacks;

CREATE OR REPLACE VIEW v_blockblast_leaderboard AS
SELECT
...
-- ============================================================
-- SELESAI! ✅ AWR v6.1
-- [x] blockblast_settings  — toggle event ON/OFF
-- [x] blockblast_scores    — skor player, upsert best score
-- [x] v_blockblast_leaderboard — view top 20
-- [x] get_dashboard_stats() update dengan blockblast stats
-- [x] notifications type + 'key_received' untuk hadiah event
-- [x] Semua policy RLS lengkap & idempotent
-- ============================================================
