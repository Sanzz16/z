...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!duration_type) return res.status(400).json({ error: 'Durasi wajib' })
  ...
  const { data: newKey, error } = await supabaseAdmin.from('keys').insert({
    ...
    expires_at: exp?.toISOString() ?? null, duration_type, is_active: true, assigned_to: null
  }).select().single()
  ...
  if (allUsers?.length) {
    await supabaseAdmin.from('notifications').insert(
      allUsers.map((u: any) => ({
        ...
        message: 'Developer membagikan key yang bisa dipakai semua orang',
